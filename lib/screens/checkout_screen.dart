...
class _CheckoutScreenState extends State<CheckoutScreen>
  ...
  List<CartItem> get _validCartItems {
    return widget.cartItems.where((item) => item.numericPrice > 0).toList();
  }

  double get _subtotal {
    return _validCartItems.fold(0.0, (sum, item) => sum + item.totalPrice);
  }
  ...
  Future<void> _processOrder() async {
    ...
    if (_validCartItems.isEmpty) {
      _showErrorSnackBar('❌ No hay productos válidos para procesar');
      return;
    }
                ...
                Flexible(
                  child: Container(
                    ...
                    child: FittedBox(
                      fit: BoxFit.scaleDown,
                      child: Text(
                        '${_validCartItems.length} ${_validCartItems.length == 1 ? 'producto' : 'productos'}',
                        style: TextStyle(
  ...
  Widget _buildOrderItem(CartItem item) {
    ...
    final isSmallScreen = screenWidth < 360;
    final isMediumScreen = screenWidth < 400;
    final isOutOfStock = item.numericPrice <= 0;
    
    return Container(
      padding: EdgeInsets.all(isSmallScreen ? 12 : 16),
      decoration: BoxDecoration(
        color: isOutOfStock ? Colors.grey[100] : backgroundColor,
        ...
        children: [
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    fontSize: isSmallScreen ? 14 : 16,
                    fontWeight: FontWeight.w600,
                    color: isOutOfStock ? Colors.grey : textPrimary,
                  ),
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    if (screenWidth > 320 && !isOutOfStock)
                      Container(
                        constraints: BoxConstraints(
          ...
          Flexible(
            child: FittedBox(
              fit: BoxFit.scaleDown,
              child: Text(
                isOutOfStock ? 'Sin stock' : item.price,
                style: TextStyle(
  ...
  Widget _buildProcessButton() {
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: _clientFoundInSAP && _validCartItems.isNotEmpty
        ...
        boxShadow: [
          BoxShadow(
            color: (_clientFoundInSAP && _validCartItems.isNotEmpty ? secondaryColor : Colors.grey).withOpacity(0.3),
            blurRadius: 20,
            offset: const Offset(0, 8),
          ),
        ],
      ...
      child: Material(
        ...
        child: InkWell(
          borderRadius: BorderRadius.circular(20),
          onTap: (_isProcessingOrder || !_clientFoundInSAP || _validCartItems.isEmpty) 
              ? null 
              : _processOrder,
          child: Center(
                    ...
                    children: [
                      ...
                      Column(
                        ...
                        children: [
                          ...
                          if (!_clientFoundInSAP && _validCartItems.isNotEmpty)
                            const Text(
