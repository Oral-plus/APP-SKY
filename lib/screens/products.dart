  ...
  class _ProductPreviewDialogState extends State<ProductPreviewDialog>
    ...
    String _obtenerMensajeEstado(String codigoSap) {
      if (widget.estadosSAP.containsKey(codigoSap)) {
        ...
        return sap.InvoiceService1.obtenerMensajeEstado(widget.estadosSAP[codigoSap]!);
      }
      return 'Producto disponible';
    }
...
class CartBottomSheet extends StatefulWidget {
  ...
  double precioNumerico = double.tryParse(item.price.replaceAll(RegExp(r'[^\d.]'), '')) ?? 0.0;

  double subtotalReal = precioNumerico * item.quantity; // precio original sin IVA
  double ivaItem = subtotalReal * 0.19;

  // 🔹 Mostramos subtotal restando el IVA
  double subtotalItem = subtotalReal - ivaItem;

  // 🔹 Total sin restar ni sumar nada (precio original)
      ...
      return ListenableBuilder(
        ...
        builder: (context, child) {
          ...
          for (var item in _cartManager.items) {
            double precioNumerico = double.tryParse(item.price.replaceAll(RegExp(r'[^\d.]'), '')) ?? 0.0;
            double subtotalItem = precioNumerico * item.quantity;
            double ivaItem = subtotalItem * 0.19;
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text(
                      '\$${subtotalGeneral.toStringAsFixed(2)}',
                      style: TextStyle(
                        fontSize: context.clampFont(16, 20, 18),
                        color: textSecondary,
                      ),
                ...
                Divider(height: context.responsive(16), thickness: 2),
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Text(
                      'Total:',
                      style: TextStyle(
                        fontSize: context.clampFont(20, 26, 22),
                        fontWeight: FontWeight.w800,
                        color: primaryBlue,
                      ),
                    ),
        ...
        child: Column(
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  colors: [primaryBlue.withOpacity(0.05), secondaryBlue.withOpacity(0.05)],
                ),
                borderRadius: BorderRadius.circular(context.responsive(16)),
