...
class PagoService {
  ...
  static Future<Map<String, dynamic>> testConnectionWithDiagnostic() async {
    ...
    if (workingUrl == null) {
      diagnostic['recommendations'].addAll([
        ...
        '3. Verifica que el puerto 6 esté libre',
  ...
  static Future<Map<String, dynamic>> getPaidInvoicesByCardCode(String cardCode) async {
    ...
    try {
      print('💰 Consultando facturas PAGADAS para CardCode: $cardCode');
      ...
      if (response.statusCode == 200) {
        ...
        if (data['success'] == true) {
          print('✅ Respuesta exitosa de la API');
          print('💰 Facturas pagadas encontradas: ${data['count']}');
          print('⏱️ Tiempo de consulta en servidor: ${data['queryTime']}ms');
          print('📊 Estadísticas: ${data['statistics']}');
        ...
        print('📭 No se encontraron facturas pagadas para CardCode: $cardCode');
        return {
          'success': true,
          'count': 0,
          'paidInvoices': [],
          ...
          'message': 'No hay facturas pagadas registradas',
        };
      } else {
        print('❌ Error HTTP ${response.statusCode}');
        print('📄 Respuesta: ${response.body}');
      ...
      print('❌ Error obteniendo facturas pagadas: $e');
